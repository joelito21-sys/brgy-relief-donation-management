    ...
    <div class="max-w-4xl mx-auto">
            ...
            <div class="p-6 md:p-8">
                <div class="flex flex-col md:flex-row items-center justify-between">
                    ...
                    <div class="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                        <p class="text-white font-semibold">Amount: <span id="header_amount_display">₱{{ number_format($amount, 2) }}</span></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="p-6 md:p-8">
                    ...
                    <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
                        <div class="mb-6">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" 
                                       id="donation_amount" 
                                       class="w-full pl-10 pr-4 py-4 text-3xl font-bold text-gray-800 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all bg-white shadow-sm"
                                       placeholder="0.00"
                                       value="{{ $amount }}"
                                       min="1" step="1">
                            </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <button type="button" onclick="setAmount(100)" class="amount-btn py-3 px-4 bg-white border-2 border-green-100 rounded-xl hover:border-green-500 hover:bg-green-50 text-green-700 font-bold text-lg transition-all transform hover:-translate-y-1 shadow-sm">₱100</button>
                                <button type="button" onclick="setAmount(500)" class="amount-btn py-3 px-4 bg-white border-2 border-green-100 rounded-xl hover:border-green-500 hover:bg-green-50 text-green-700 font-bold text-lg transition-all transform hover:-translate-y-1 shadow-sm">₱500</button>
                                <button type="button" onclick="setAmount(1000)" class="amount-btn py-3 px-4 bg-white border-2 border-green-100 rounded-xl hover:border-green-500 hover:bg-green-50 text-green-700 font-bold text-lg transition-all transform hover:-translate-y-1 shadow-sm">₱1,000</button>
                                <button type="button" onclick="setAmount(5000)" class="amount-btn py-3 px-4 bg-white border-2 border-green-100 rounded-xl hover:border-green-500 hover:bg-green-50 text-green-700 font-bold text-lg transition-all transform hover:-translate-y-1 shadow-sm">₱5,000</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-100 rounded-xl p-6 mb-8">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between py-2 border-b border-gray-200">
                            ...
                            <span class="font-medium" id="summary_donation_amount">₱{{ number_format($amount, 2) }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-200">
                        ...
                        <div class="flex justify-between py-3 font-bold text-lg">
                            ...
                            <span class="text-green-600" id="summary_total_amount">₱{{ number_format($amount, 2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <!-- Pay Now Button -->
                <div class="mb-8">
                    <a href="{{ route('donor.donations.cash.gateway', ['method' => 'bank']) }}" 
                       id="pay_now_btn"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="bg-white p-4 rounded-lg border border-gray-200">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <p class="font-medium text-gray-900">{{ config('payment_accounts.bank.bpi.account_name') }}</p>
                                        <button type="button" class="text-green-600 hover:text-green-800 ml-2" onclick="copyToClipboard('{{ config('payment_accounts.bank.bpi.account_name') }}')">
                                ...
                                <div class="bg-white p-4 rounded-lg border border-gray-200">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <p class="font-mono font-medium text-gray-900">{{ config('payment_accounts.bank.bpi.account_number') }}</p>
                                        <button type="button" class="text-green-600 hover:text-green-800 ml-2" onclick="copyToClipboard('{{ str_replace(' ', '', config('payment_accounts.bank.bpi.account_number')) }}')">
                                            <i class="fas fa-copy"></i>
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div class="bg-gradient-to-br from-red-50 to-pink-50 p-6 rounded-xl border border-red-100 shadow-sm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="bg-white p-4 rounded-lg border border-gray-200">
                                    <p class="text-sm text-gray-500 mb-1">Account Name</p>
                                    <div class="flex items-center justify-between">
                                        <p class="font-medium text-gray-900">{{ config('payment_accounts.bank.bdo.account_name') }}</p>
                                        <button type="button" class="text-green-600 hover:text-green-800 ml-2" onclick="copyToClipboard('{{ config('payment_accounts.bank.bdo.account_name') }}')">
                                            <i class="fas fa-copy"></i>
                                        </button>
                                    </div>
                                </div>
                                <div class="bg-white p-4 rounded-lg border border-gray-200">
                                    <p class="text-sm text-gray-500 mb-1">Account Number</p>
                                    <div class="flex items-center justify-between">
                                        <p class="font-mono font-medium text-gray-900">{{ config('payment_accounts.bank.bdo.account_number') }}</p>
                                        <button type="button" class="text-green-600 hover:text-green-800 ml-2" onclick="copyToClipboard('{{ str_replace(' ', '', config('payment_accounts.bank.bdo.account_number')) }}')">
                                            <i class="fas fa-copy"></i>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="bg-white p-4 rounded-lg border border-gray-200">
                                    <p class="text-sm text-gray-500 mb-1">Branch</p>
                                    <p class="font-medium text-gray-900">{{ config('payment_accounts.bank.bdo.branch') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="bg-gradient-to-br from-blue-50 to-cyan-50 p-6 rounded-xl border border-blue-100 shadow-sm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="bg-white p-4 rounded-lg border border-gray-200">
                                    <p class="text-sm text-gray-500 mb-1">Account Name</p>
                                    <div class="flex items-center justify-between">
                                        <p class="font-medium text-gray-900">{{ config('payment_accounts.bank.metrobank.account_name') }}</p>
                                        <button type="button" class="text-green-600 hover:text-green-800 ml-2" onclick="copyToClipboard('{{ config('payment_accounts.bank.metrobank.account_name') }}')">
                                            <i class="fas fa-copy"></i>
                                        </button>
                                    </div>
                                </div>
                                <div class="bg-white p-4 rounded-lg border border-gray-200">
                                    <p class="text-sm text-gray-500 mb-1">Account Number</p>
                                    <div class="flex items-center justify-between">
                                        <p class="font-mono font-medium text-gray-900">{{ config('payment_accounts.bank.metrobank.account_number') }}</p>
                                        <button type="button" class="text-green-600 hover:text-green-800 ml-2" onclick="copyToClipboard('{{ str_replace(' ', '', config('payment_accounts.bank.metrobank.account_number')) }}')">
                                            <i class="fas fa-copy"></i>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="bg-white p-4 rounded-lg border border-gray-200">
                                    <p class="text-sm text-gray-500 mb-1">Branch</p>
                                    <p class="font-medium text-gray-900">{{ config('payment_accounts.bank.metrobank.branch') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-4">
                            ...
                            <ol class="list-decimal list-inside text-green-700 space-y-2">
                                <li>Transfer your donation of <span class="font-bold" id="instruction_amount">₱{{ number_format($amount, 2) }}</span> to any of the bank accounts above</li>
                ...
                <!-- Payment Form -->
                <form action="{{ route('donor.donations.cash.process', ['method' => 'bank']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
