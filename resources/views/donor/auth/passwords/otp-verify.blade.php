    ...
    <div class="max-w-md w-full space-y-8">
        <div>
            ...
            <p class="mt-2 text-center text-sm text-gray-600">
                Please enter the 6-digit code sent to <strong>{{ session('reset_donor_email') }}</strong>
            </p>
        </div>
        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('donor.password.otp.verify.process') }}">
            @csrf
            ...
            <div class="flex items-center justify-between text-sm">
                <form action="{{ route('donor.password.otp.send') }}" method="POST" class="inline">
                    @csrf
                    <input type="hidden" name="email" value="{{ session('reset_donor_email') }}">
                    <button type="submit" class="font-medium text-teal-600 hover:text-teal-500 bg-transparent border-0 p-0 cursor-pointer">
                        Resend Code
                    </button>
                </form>
                
                <a href="{{ route('donor.password.request') }}" class="font-medium text-gray-600 hover:text-gray-500">
                    Change Email
                </a>
            </div>
        </form>
