    ...
    <div class="stat-card bg-gradient-to-r from-violet-500 to-violet-600 rounded-xl p-6 text-white shadow-lg transform hover:-translate-y-1 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold mt-2">{{ number_format($requests->total()) }}</p>
            </div>
    ...
    <div class="stat-card bg-gradient-to-r from-amber-500 to-amber-600 rounded-xl p-6 text-white shadow-lg transform hover:-translate-y-1 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold mt-2">{{ number_format($requests->where('status', 'pending')->count()) }}</p>
            </div>
            <div class="bg-white/20 rounded-lg p-3 backdrop-blur-sm">
    ...
    <div class="stat-card bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-xl p-6 text-white shadow-lg transform hover:-translate-y-1 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold mt-2">{{ number_format($requests->where('status', 'approved')->count()) }}</p>
            </div>
            <div class="bg-white/20 rounded-lg p-3 backdrop-blur-sm">
    ...
    <div class="stat-card bg-gradient-to-r from-cyan-500 to-cyan-600 rounded-xl p-6 text-white shadow-lg transform hover:-translate-y-1 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold mt-2">{{ number_format($requests->where('status', 'ready_for_pickup')->count()) }}</p>
            </div>
            <div class="bg-white/20 rounded-lg p-3 backdrop-blur-sm">
    ...
    <div class="p-6 border-b border-gray-100 bg-gray-50/50">
        ...
        <div class="mt-6 flex flex-col lg:flex-row gap-4">
            ...
            <div class="flex gap-2">
                <input type="date" id="dateFrom" value="{{ request('date_from') }}" class="px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 text-gray-700 bg-white">
                <input type="date" id="dateTo" value="{{ request('date_to') }}" class="px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 text-gray-700 bg-white">
            </div>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors group">
                    <td class="px-6 py-4">
                        <input type="checkbox" class="request-checkbox rounded border-gray-300 text-violet-600 focus:ring-violet-500 w-4 h-4" value="{{ $request->id }}">
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <span class="font-bold text-gray-900">{{ $request->request_number }}</span>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <div class="h-9 w-9 rounded-full bg-violet-100 flex items-center justify-center text-violet-600 font-bold text-sm mr-3">
                                {{ substr($request->resident->name ?? '?', 0, 1) }}
                            </div>
                            <div>
                                <div class="font-bold text-gray-900">{{ $request->resident->name ?? 'N/A' }}</div>
                                <div class="text-xs text-gray-500">{{ $request->resident->contact_number ?? 'No contact' }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            @if(in_array($request->status, ['approved', 'ready_for_pickup']))
                                <a href="{{ route('admin.relief-requests.print', $request) }}" target="_blank" 
                                   class="p-1.5 text-violet-600 hover:bg-violet-50 rounded" title="Print Claim Slip">
                                    <i class="fas fa-print"></i>
                                </a>
                            @endif
                            
                            <a href="{{ route('admin.relief-requests.show', $request) }}" 
                               class="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="View Details">
                                <i class="fas fa-eye"></i>
                            </a>
                            
                            @if($request->isPending())
                                <button onclick="approveRequest({{ $request->id }}, '{{ addslashes($request->resident->name) }}')" 
                                        class="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded" title="Approve">
                                    <i class="fas fa-check"></i>
                                </button>
                                <button onclick="openRejectModal({{ $request->id }}, '{{ addslashes($request->resident->name) }}')"
...
<div id="createRequestModal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl w-full">
            ...
            <form action="{{ route('admin.relief-requests.store') }}" method="POST" class="p-6">
                @csrf
...
<script>
    ...
    document.getElementById('applyFilters').addEventListener('click', function() {
        ...
        window.location.href = `{{ route('admin.relief-requests.index') }}?search=${search}&status=${status}&delivery_method=${method}&date_from=${from}&date_to=${to}`;
    });

    document.getElementById('clearFilters').addEventListener('click', function() {
        window.location.href = `{{ route('admin.relief-requests.index') }}`;
    });
    ...
    document.querySelectorAll('.bulk-action').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            // Re-implementing the fetch call:
             fetch('{{ route('admin.relief-requests.bulk') }}', {
                method: 'POST',
                headers: {
