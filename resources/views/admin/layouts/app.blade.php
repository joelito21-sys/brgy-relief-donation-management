<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" x-data="{ sidebarOpen: window.innerWidth >= 1024, collapsed: false }" @resize.window="sidebarOpen = window.innerWidth >= 1024">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="csrf-token" content="{{ csrf_token() }}">

    <title>{{ config('app.name', 'Flood Control') }} - Admin Panel</title>
            ...
            <div class="p-4 border-b border-violet-400">
                <div class="flex items-center">
                    ...
                    <div class="ml-3 overflow-hidden">
                        ...
                        <p class="text-xs text-violet-200 truncate">{{ Auth::guard('admin')->user()->email }}</p>
                    </div>
                </div>
            </div>
                    ...
                    @foreach($navigation as $item)
                        <div class="nav-item {{ $item['active'] ? 'active' : '' }}">
                            @if(isset($item['submenu']))
                                <!-- Submenu Item -->
                                <div x-data="{ submenuOpen: {{ $item['active'] ? 'true' : 'false' }} }">
                                    ...
                                    <!-- Submenu -->
                                    <div x-show="submenuOpen" class="mt-2 ml-4 space-y-1" style="display: {{ $item['active'] ? 'block' : 'none' }};">
                                        @foreach($item['submenu'] as $subitem)
                                            <a href="{{ route($subitem['route']) }}" 
                                               class="nav-link text-sm {{ $subitem['active'] ? 'active' : '' }} bg-violet-500 bg-opacity-20">
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="relative" x-data="{ open: false }">
                                    ...
                                    <div x-show="open" @click.away="open = false" 
                                         ...
                                         style="display: none;">
                                        <a href="{{ route('admin.profile') }}" class="block px-4 py-2 hover:bg-violet-50 hover:text-violet-600 transition-colors">
                                            <i class="fas fa-user mr-2"></i> Profile
                                        </a>
                                        <a href="{{ route('admin.settings') }}" class="block px-4 py-2 hover:bg-violet-50 hover:text-violet-600 transition-colors">
