        ...
        <div class="d-flex justify-content-between align-items-start">
            <div class="d-flex align-items-center gap-4">
                <div class="profile-avatar text-center">
                    <img src="https://ui-avatars.com/api/?name={{ urlencode($resident->first_name . ' ' . $resident->last_name) }}&background=random&size=128" 
                         alt="Profile" class="rounded-circle" width="80" height="80">
                </div>
                <div>
                    <h1 class="display-6 fw-bold mb-1">{{ $resident->first_name }} {{ $resident->last_name }}</h1>
                    <div class="d-flex align-items-center gap-3">
                        <span class="opacity-75"><i class="fas fa-envelope me-2"></i>{{ $resident->email }}</span>
                        <span class="opacity-75">|</span>
                        <span class="opacity-75"><i class="fas fa-phone me-2"></i>{{ $resident->phone }}</span>
                    </div>
                </div>
            </div>
            <a href="{{ route('admin.residents.index') }}" class="btn btn-light btn-sm shadow-sm">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="info-value">{{ $resident->id_type }}</div>
                        </div>
                        <div class="col-md-4">
                            <div class="info-label">Registration Date</div>
                            <div class="info-value">{{ $resident->created_at->format('M d, Y') }}<br><small class="text-muted">{{ $resident->created_at->format('h:i A') }}</small></div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            @if($resident->valid_id_front)
                                @php $frontUrl = asset('storage/' . str_replace('\\', '/', $resident->valid_id_front)); @endphp
                                <div class="id-card-preview" onclick="window.open('{{ $frontUrl }}', '_blank')">
                        ...
                        <div class="col-md-6">
                            ...
                            @if($resident->valid_id_back)
                                @php $backUrl = asset('storage/' . str_replace('\\', '/', $resident->valid_id_back)); @endphp
                                <div class="id-card-preview" onclick="window.open('{{ $backUrl }}', '_blank')">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="d-flex flex-column gap-3">
                                <div>
                                    ...
                                    <div class="info-value">{{ $resident->house_number ? $resident->house_number . ', ' : '' }}{{ $resident->address }}</div>
                                </div>
                                <div class="row">
                                    <div class="col-6">
                                        <div class="info-label">Barangay</div>
                                        <div class="info-value">{{ $resident->barangay }}</div>
                                    </div>
                                    <div class="col-6">
                                        <div class="info-label">City</div>
                                        <div class="info-value">{{ $resident->city }}</div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-6">
                                        <div class="info-label">Province</div>
                                        <div class="info-value">{{ $resident->province }}</div>
                                    </div>
                                    <div class="col-6">
                                        <div class="info-label">Country</div>
                                        <div class="info-value">{{ $resident->country }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6 border-start ps-md-4">
                            ...
                            <div class="d-flex flex-column gap-3">
                                <div>
                                    ...
                                    <div class="info-value">{{ $resident->emergency_contact_name }}</div>
                                </div>
                                <div>
                                    <div class="info-label">Contact Number</div>
                                    <div class="info-value text-danger fw-bold">{{ $resident->emergency_contact_phone }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body p-4 text-center">
                    ...
                    @if($resident->isPending())
                        ...
                        <div class="d-grid gap-2">
                            <form method="POST" action="{{ route('admin.residents.approve', $resident) }}" class="d-grid">
                                @csrf
                                <button type="submit" class="btn btn-success py-2 fw-bold" onclick="return confirm('Approve this resident registration?')">
                                    <i class="fas fa-check-circle me-2"></i> Approve
                                </button>
                            </form>
                            <button type="button" class="btn btn-outline-danger py-2 fw-bold" onclick="showRejectModal({{ $resident->id }})">
                    ...
                    @elseif($resident->isApproved())
                        ...
                        <p class="text-success small mb-0">Access granted on {{ $resident->updated_at->format('M d, Y') }}</p>
                    ...
                    @elseif($resident->isRejected())
                        ...
                        @if($resident->rejection_reason)
                            <div class="bg-light rounded p-3 text-start border-start border-4 border-danger">
                                ...
                                <span class="text-dark">{{ $resident->rejection_reason }}</span>
                            </div>
                        @endif
                    @endif
                </div>
                     ...
                     <div class="row g-2">
                         <div class="col-6">
                             <div class="p-3 rounded bg-light text-center border">
                                 <h3 class="fw-bold text-primary mb-0">{{ $resident->reliefRequests()->count() }}</h3>
                                 <small class="text-muted">Requests</small>
                             </div>
                         </div>
                         <div class="col-6">
                             <div class="p-3 rounded bg-light text-center border">
                                 <h3 class="fw-bold text-success mb-0">{{ $resident->receivedDonations()->count() }}</h3>
            ...
            <div class="card info-card">
                <div class="card-body p-4">
                    ...
                    @if($resident->activityLogs && $resident->activityLogs->count() > 0)
                        <div class="ps-2">
                            @foreach($resident->activityLogs->take(5) as $log)
                                <div class="timeline-item">
                                    <div class="timeline-dot"></div>
                                    <div class="timeline-date">{{ $log->created_at->format('M d, h:i A') }}</div>
